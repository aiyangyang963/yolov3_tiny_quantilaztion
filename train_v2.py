...
from yolo3.model_v2 import preprocess_true_boxes, yolo_body, yolo_loss, yolo_eval_v2,  tiny_yolo_body
...
parser.add_argument('--classes_file', type=str, help='The .txt file include dataset <classes>',
                    default='model_data/shrimp.names')
#parser.add_argument('--anchors_file', type=str, help='The .txt file include yolo anchors type',
#                    default='model_data/yolo_anchors.txt')
#  yolo_anchors.txt   / tiny_yolo_anchors_shrimp.txt
parser.add_argument('--anchors_file', type=str, help='The .txt file include yolo anchors type',
                    default='model_data/tiny_yolo_anchors_shrimp.txt')
# Train File    train_kr.txt
parser.add_argument('--yolo_train_file', type=str, help='The .txt file include <img path>, <bbox>, <class>',
                    default= './val_dehze_kr.txt')
# Evaluate file val_dehze_kr.txt
...
class Yolo(object):
    def __init__(self):
        ...
        self.input_shape = (512, 512)  # multiple of 32, hw
        ...
        if self.is_tiny_version:
            self.yolo_model, self.yolo_body = self.create_tiny_model(load_pretrained=False,weights_path='model_data/yolov3-tiny_obj_final123.h5')
        else:
            self.yolo_model, self.yolo_body = self.create_model(load_pretrained=False,yolo_weights_path='model_data/yolo_weights_00.h5')
    ...
    def create_tiny_model(self, load_pretrained=True, freeze_body=2,
        ...
        print('Create Tiny YOLOv3 model with {} anchors and {} classes.'.format(num_anchors, self.num_classes))
        ...
        model_loss = Lambda(yolo_loss, output_shape=(1,), name='yolo_loss',
                ...
                [*model_body.output, *y_true])
        yolo_tiny_model = Model(inputs=[image_input, *y_true], outputs=model_loss)
        print('==========================================   Yolo-Tiny Body   =========================================')
        yolo_tiny_model.summary()

        return yolo_tiny_model, model_body
    ...
    def train(self):
        ...
        print('Evaluate mAP')
        if self.is_tiny_version:
            _ = self.eval_tiny(-1, self.val_images, self.tmp_gt_files_path, 'mAP')
        else:
        ...
        for step in range(start, end):
            ...
            if step % epoch == 0:
                ...
                if self.is_tiny_version:
                    ...
                    mAP = self.eval_tiny(step // epoch, self.val_images, self.tmp_gt_files_path, 'mAP')
                else:
                    mAP = self.eval(step // epoch, self.val_images, self.tmp_gt_files_path, 'mAP')
        ...
        for step in range(start, end):
            ...
            if step % epoch == 0:
                ...
                if self.is_tiny_version:
                    print("is_tiny_version")
                    mAP = self.eval_tiny(step - start // epoch, self.val_images, self.tmp_gt_files_path, 'mAP')
                else:
                    mAP = self.eval(step - start // epoch, self.val_images, self.tmp_gt_files_path, 'mAP')
    ...
    def eval_tiny(self, step, eval_images_path, ground_truth_path, tag='image', is_save_images=True):
        # Add the class predict temp dict
        class_pred_tmp = {}
        for class_name in self.class_names:
            class_pred_tmp[class_name] = []
        ...
        for start in range(0, len(eval_images_path), self.step2_batch_size):
            ...
            for i, out in enumerate(zip(out_bboxes_1, out_bboxes_2)):
                # Predict
                out_boxes, out_scores, out_classes = self.sess.run(
                    [self.boxes, self.scores, self.classes],
                    feed_dict={
                ...
                ord_h = image.shape[0]
                ord_w = image.shape[1]
                new_h = int(image.shape[0] * 2 / 4)
                new_w = int(image.shape[1] * 2 / 4)
                image = cv2.resize(image, (new_w, new_h))
                for o, c in enumerate(out_classes):
                    predicted_class = self.class_names[c]
                    box = out_boxes[o]
                    ...
                    # Draw image
                    new_top, new_left, new_bottom, new_right = (box * 2 / 4).astype('int32')
                    if files_id[i] in self.eval_save_images_id:
                        label = '{} {:.2f}'.format(predicted_class, score)
                        # print(label, (left, top), (right, bottom))
                        color = self.colors[c]
